...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <!-- has quantity -->
            <div class="form-group">
                <label for="has_quantity">{{ __('product.has_quantity') }}</label>
                <select name="has_quantity" class="form-control @error('has_quantity') is-invalid @enderror" id="has_quantity">
                    <option value="1" {{ old('has_quantity', $product->has_quantity) === 1 ? 'selected' : ''}}>{{ __('common.Yes') }}</option>
                    <option value="0" {{ old('has_quantity', $product->has_quantity) === 0 ? 'selected' : ''}}>{{ __('common.No') }}</option>
                </select>
                @error('has_quantity')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
